...
\ref{rq1}章で述べた，「機能拡張に伴うテストコード追加」「機能削除に伴うテストコード削除」「APIの仕様変更に伴うテストコードのアサーション変更」に対して，それぞれ条件を定義する．

\subsubsection{機能拡張に伴うテストコード追加}
テストコード追加を定義するためにテストコードを定義する．



\subsection{各テストコード変更内容の定義}
\ref{rq1}章で述べた，各テストコード変更内容を定義するために，JavaScript言語におけるテストコードの種類をまとめる．JavaScript言語では，テストコードを書く際にフレームワークを使うことが多く，フレームワーク毎にテストコードの書き方は異なる．本調査では，State of JavaScript 2022\footnote{\url{https://2022.stateofjs.com/}}で紹介されている主要なテストフレームワーク13件のうち，単体テストで使われるフレームワーク5件(Jest\footnote{\url{https://jestjs.io/}},Mocha\footnote{\url{https://mochajs.org/}},AVA\footnote{\url{https://github.com/avajs/ava}},Jasmine\footnote{\url{https://jasmine.github.io/}},Vitest\footnote{\url{https://vitest.dev/}})を考慮する．テストフレームワーク毎にテストスイート，テストケースの記述形式は大きく変わらないが，アサーションの書き方は異なる場合がある．テストフレームワーク毎のアサーションの書き方は，大きく振る舞い駆動の記述形式(BDD)と，テスト駆動の記述形式(TDD)の2つに大別できる．BDDの記述形式の例をProgram\ref{bdd.test.js}，TDDの記述形式の例をProgram\ref{tdd.test.js}で示す．
...
変更情報の取得には，GumTree\cite{gumtree}を使用する．GumTreeは，入力として変更前のソースファイルと変更後のソースファイルを受け取り，ASTのノード単位の編集スクリプトを出力する．


\subsubsection{テストコードの定義}
...
抽象構文木（AST）は，ソースコードを構文解析して得られる木構造のデータである．

松田研究では，it or testという関数名で，第一引数が文字列(テスト内容を示すラベル)，第二引数が関数呼び出しの場合，それをテストケースとしています．
本研究では，テストスイートの追加・削除を含めるため，テストスイートを示すdescribeという関数名をキーに加え，describe or it or testという関数名をテストコードとして判定します．
...
\subsection{各テストコード変更内容の定義}
\subsubsection{GumTreeによる差分検出}



\subsection{テストコード変更内容の検出}
